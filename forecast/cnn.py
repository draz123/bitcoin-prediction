...
def forecast_price(start_date, stop_date, data):
  ...
  if (start_date == None and stop_date == None):
    ...
    timeseries = data['Close']                   # The timeseries f(t) = t
    working_pred = evaluate_timeseries(timeseries, window_size)
    test_size = int(len(data) * 0.3)
    predictDates = data.tail(test_size).index
    formattedRealValues = pd.Series(data['Close'].tail(test_size))
    formattedForecastValues = working_pred
    ...
    jsonData['predictedValues'] = pd.Series(functools.reduce(list.__add__, formattedForecastValues.tolist())).to_json(orient='values')
    jsonData['realValues'] = []
    ...
    with open('resources/cnn.json', 'w') as outfile:
...
def get_cnn_json():
  with open('resources/cnn.json') as f:
    data = json.load(f)
